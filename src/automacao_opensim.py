...
# ==========================================
# A raiz do projeto é identificada automaticamente a partir da localização
# deste script, eliminando a necessidade de caminhos absolutos.
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parent

DATASET_DIR = PROJECT_ROOT / "data" / "raw"
TEMPLATES_DIR = PROJECT_ROOT / "templates"
PACIENTES_DIR = PROJECT_ROOT / "data" / "processed"
RESULTS_DIR = PROJECT_ROOT / "results"
REPORT_FILE = RESULTS_DIR / "pipeline_report.txt"

C3D_TO_TRC_SCRIPT = SCRIPT_DIR / "c3d_to_trc.py"
PLOT_MUSCLE_SCRIPT = SCRIPT_DIR / "plot_muscle.py"
PLOT_COM_SCRIPT = SCRIPT_DIR / "plot_com.py"
PLOT_KIN_SCRIPT = SCRIPT_DIR / "plotter_bonito.py"
...
def check_opensim_cmd(custom_path=None):
    """Localiza o executável opensim-cmd.

    O caminho pode ser informado por --opensim. Caso contrário, o executável
    deve estar disponível na variável de ambiente PATH.
    """
    if custom_path:
...
def main():
    ...
    template_ik = TEMPLATES_DIR / "Setup_IK_Template.xml"
    template_analyze = TEMPLATES_DIR / "Setup_Analyze_Template.xml"

    # Criando diretórios locais de saída
    ...
    # Coletar pastas de participantes
    young_dir = DATASET_DIR / "Young"
    old_dir = DATASET_DIR / "Old"

    # Compatibilidade com nomes de pasta em letras minúsculas.
    if not young_dir.exists():
        young_dir = DATASET_DIR / "young"
    if not old_dir.exists():
        old_dir = DATASET_DIR / "older"

    pacientes_young = sorted([p for p in young_dir.glob("*") if p.is_dir()])[:15]
    pacientes_old = sorted([p for p in old_dir.glob("*") if p.is_dir()])[:15]
